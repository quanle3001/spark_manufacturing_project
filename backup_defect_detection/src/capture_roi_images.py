...
def draw_counts_panel(frame, counts, current_class, next_number):
    ...
    cv2.putText(
        frame,
        f"Current: {current_class} | Next: {current_class}_{next_number:06d}.jpg",
        (x, y),
...
def main():
    ...
    parser.add_argument("--class-name", required=True, choices=CLASSES)
    ...
    parser.add_argument("--count", type=int, default=0, help="Number of NEW images to capture. 0 means manual unlimited.")
